...
class Region(tree(), ModelSQL, ModelView):
    ...
    divisions = fields.One2Many(
        'census.region', 'parent', "Divisions", readonly=True)
    places = fields.One2Many(
        'census.place', 'region', "Places",
        filter=[('parent', '=', None)], readonly=True)
...
class Place(tree(), DeactivableMixin, ModelSQL, ModelView):
    ...
    region = fields.Many2One('census.region', "Region", readonly=True)
